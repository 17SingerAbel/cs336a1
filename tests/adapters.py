...
def run_scaled_dot_product_attention(
    ...
    d_k = Q.shape[-1]
    base_score = einsum(Q, K, '... queries d_k, ... keys d_k -> ... queries keys')
    attention_weights = base_score / (d_k ** 0.5)
    if mask is not None:
        attention_weights += torch.where(mask, 0, -torch.inf)
    return run_softmax(attention_weights, -1) @ V
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    max_entity = torch.amax(in_features, dim=dim, keepdim=True)
    return torch.exp(in_features - max_entity) / torch.sum(torch.exp(in_features - max_entity), dim=dim, keepdim=True)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return train_bpe_heap(input_path=input_path, vocab_size=vocab_size, special_tokens=special_tokens)
